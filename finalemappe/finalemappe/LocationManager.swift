    ...
    func removePoints(){
        if let lengthData = UserDefaults.standard.data(forKey: "length") {
            do {
                let decodedLength = try JSONDecoder().decode(Int.self, from: lengthData)
                for (index) in 0..<decodedLength {
                    UserDefaults.standard.removeObject(forKey: "name\(index)")
                    UserDefaults.standard.removeObject(forKey: "lat\(index)")
                    UserDefaults.standard.removeObject(forKey: "long\(index)")
                    UserDefaults.standard.removeObject(forKey: "color\(index)")
